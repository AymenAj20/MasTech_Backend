...
// Methode qui changer l'etat du chantier
exports.changerEtatChantier = async function (req, res) {
  try {
    const chantier = await Chantier.findByPk(req.params.id);
    if (!chantier) {
      return res.status(404).send({ message: " Chantier non trouvé !!" });
    }
    await chantier.update({etat:!chantier.etat});
    return res.status(200).send({ message: "La mise à jour a été réalisée avec succès." });
  } catch (error) {
...
exports.getTasks = async function (req, res) {
  try {
    const chantier = await Chantier.findByPk(req.params.id, {
      include: { model: Tache, where: { statut:false} },
    });
...
exports.addChantier = async function (req, res) {
  ...
  try {
    ...
    res.status(201).send({ message:"Chantier a créé"});
  } catch (error) {
    console.log(error);
  }
};
